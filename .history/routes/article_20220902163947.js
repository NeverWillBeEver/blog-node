...
import { responseClient, timestampToTime } from '../util/util.js';
...
export const delArticle = (req, res) => {
  let { id } = req.body;
...
// 后台文章列表
export const getArticleListAdmin = (req, res) => {
  let keyword = req.query.keyword || null;
  let state = req.query.state || '';
  let likes = req.query.likes || '';
  let pageNum = parseInt(req.query.pageNum) || 1;
...
// 文章详情
export const getArticleDetailByType = (req, res) => {
  let { type } = req.body;
  if (!type) {
    responseClient(res, 200, 1, '文章不存在 ！');
    return;
